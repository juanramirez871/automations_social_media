...
export async function GET(request) {
  try {
    ...
    console.log('🔄 Intercambiando code por access_token usando Facebook Graph API...');
    ...
    if (!tokenRes2.ok || tokenJson.error_type || tokenJson.error_message) {
      const msg = tokenJson?.error_message || `Token exchange failed (${tokenRes2.status})`;
      const html = `<!DOCTYPE html><html><body>
    ...
    console.log('✅ Long-lived token obtenido, expira en:', longLivedExpiresIn, 'segundos');
    ...
    if (!pagesRes.ok || pagesJson.error) {
      ...
      const msg = pagesJson?.error?.message || 'Error obteniendo páginas de Facebook';
      const html = `<!DOCTYPE html><html><body>
    ...
    console.log('📄 Respuesta completa de páginas:', JSON.stringify(pagesJson, null, 2));
    ...
    if (pages.length === 0) {
      console.log('📄 No se encontraron páginas directas, consultando Business Manager...');
      ...
      console.log('🏢 Respuesta de Business Manager:', JSON.stringify(businessJson, null, 2));
      ...
      if (businessRes.ok && businessJson.data && businessJson.data.length > 0) {
        ...
        console.log('📄 Páginas del Business Manager:', JSON.stringify(businessPagesJson, null, 2));
        
        if (businessPagesRes.ok && businessPagesJson.data && businessPagesJson.data.length > 0) {
          // Usar las páginas del Business Manager
          pages.push(...businessPagesJson.data);
          console.log('✅ Páginas encontradas en Business Manager:', pages.length);
        }
      }
      
      // Si aún no hay páginas después de consultar Business Manager
      if (pages.length === 0) {
        const msg = 'No se encontraron páginas de Facebook. Necesitas una página conectada a Instagram Business.';
        const html = `<!DOCTYPE html><html><body>
    ...
    if (!igAccountRes.ok || igAccountJson.error) {
      ...
      const msg = igAccountJson?.error?.message || 'Error obteniendo cuenta de Instagram Business';
      const html = `<!DOCTYPE html><html><body>
    ...
    if (!igBusinessAccount) {
      const msg = 'Esta página de Facebook no tiene una cuenta de Instagram Business conectada.';
      const html = `<!DOCTYPE html><html><body>
        <p style="font-family: ui-sans-serif, system-ui; color:#111">${msg}</p>
        <script>
          if (window.opener) {
