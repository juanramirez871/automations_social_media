...
export const saveMessageToDB = async ({ userId, role, content = "", type, attachments, meta /* name, messageId */ }) => {
  ...
  try {
    ...
    if (/column\s+\w+\s+does\s+not\s+exist/i.test(msg) || /Could not find the '.*' column/i.test(msg)) {
      try {
        const { error } = await supabase.from("messages").insert(baseRow);
        if (error) throw error;
        console.warn("Guardado con payload mínimo por columnas faltantes en messages");
        return true;
      } catch (e2) {
