...
async function fetchProfileStatusOnce(userId) {
  ...
  const p = (async () => {
    const { data, error } = await supabase
      .from('profiles')
      .select(
        'instagram_username, facebook_access_token, facebook_user_id, facebook_granted_scopes, youtube_access_token, youtube_channel_id, youtube_channel_title, tiktok_access_token, tiktok_open_id, tiktok_granted_scopes'
      )
      .eq('id', userId)
      .maybeSingle();
  ...
  return (
    <div className='flex flex-col gap-2 items-center justify-between rounded-xl border border-gray-200 bg-white p-3'>
      ...
      >
        {working ? (
          <span
            className='size-3.5 rounded-full border-2 border-white/60 border-t-transparent animate-spin'
            aria-hidden='true'
          ></span>
        ) : (
...
export const PlatformsWidget = () => {
  ...
  const Badge = ({ ok }) => (
    <span
      className={`ml-auto inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ${ok ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-gray-100 text-gray-600 border border-gray-200'}`}
    >
      <span
        className={`size-1.5 rounded-full ${ok ? 'bg-green-500' : 'bg-gray-400'}`}
      ></span>
        ...
        <div className='group flex items-center gap-3 rounded-xl border border-blue-100 bg-gradient-to-br from-white to-blue-50/30 p-3 hover:shadow-md transition-all'>
          <div className='relative size-10 shrink-0 rounded-full bg-[#1877F2] flex items-center justify-center shadow-inner'>
            <svg viewBox='0 0 24 24' className='size-5' aria-hidden='true'>
              <path
                fill='#fff'
                d='M13.4 21v-7h2.3l.4-2.7h-2.7v-1.7c0-.8.3-1.3 1.3-1.3h1.5V5c-.3 0-1.1-.1-2.1-.1-2 0-3.4 1.2-3.4 3.5v2h-2.3V14h2.3v7h2.7z'
              />
            </svg>
          </div>
        ...
        <div className='group flex items-center gap-3 rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-3 hover:shadow-md transition-all'>
          <div className='relative size-10 shrink-0 rounded-full bg-black flex items-center justify-center shadow-inner'>
            <svg viewBox='0 0 24 24' className='size-5' aria-hidden='true'>
              <path
                fill='#fff'
                d='M16.5 7.5c1.1.9 2.4 1.4 3.8 1.5v2.3c-1.6-.1-3.1-.7-4.4-1.7v4.9c0 2.8-2.2 5-5 5s-5-2.2-5-5 2.2-5 5-5c.4 0 .7 0 1 .1v2.4c-.3-.1-.6-.1-1-.1-1.5 0-2.7 1.2-2.7 2.7S9.4 20 11 20s2.7-1.2 2.7-2.7V4h2.8v3.5z'
              />
            </svg>
          </div>
          <div className='min-w-0'>
            ...
            <p className='text-xs text-gray-500 truncate'>
              {status.tiktok.connected && status.tiktok.openId
                ? `OpenID: ${status.tiktok.openId}`
                : 'Clips y tendencias'}
            </p>
          </div>
        </div>
...
export const PostPublishWidget = ({
  ...
  const [targets, setTargets] = useState(() =>
    makeInitialTargets(defaultSelected)
  );
  ...
  const chip = (k, label, color) => (
    ...
    >
      <span
        className={`size-1.5 rounded-full ${targets[k] ? color.dotActive : color.dotInactive}`}
      ></span>
      {label}
    </button>
  );

  const selected = ['instagram', 'facebook', 'youtube', 'tiktok'].filter(
    k => targets[k]
  );
  ...
  return (
    <div className='flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4'>
      ...
      <p className='text-xs text-gray-600'>
        Selecciona una o varias plataformas para continuar.
      </p>
      ...
      <div className='flex items-center justify-end gap-2'>
        <span className='text-[11px] text-gray-500'>
          Selección: {selected.join(', ') || 'ninguna'}
        </span>
        <button
          type='button'
          onClick={() =>
...
export const CaptionSuggestWidget = ({
  caption = '',
  onAccept,
  onRegenerate,
  onCustom,
    ...
    <div className='flex flex-col gap-3 rounded-xl border border-emerald-200 bg-white p-4'>
      ...
      <div className='flex items-center justify-end gap-2'>
        ...
        >
          {active === 'regenerate' ? (
            <span
              className='size-3.5 rounded-full border-2 border-gray-700/40 border-t-transparent animate-spin'
              aria-hidden='true'
            ></span>
          ) : (
            <svg viewBox='0 0 24 24' className='size-4' aria-hidden='true'>
              <path
                fill='currentColor'
                d='M12 6v3l4-4-4-4v3C7.6 4 4 7.6 4 12c0 1.7.6 3.3 1.6 4.6l1.5-1.3C6.4 14.5 6 13.3 6 12c0-3.3 2.7-6 6-6zm6.4-.6L16.9 6.7C17.6 7.5 18 8.7 18 10c0 3.3-2.7 6-6 6v-3l-4 4 4 4v-3c4.4 0 8-3.6 8-8 0-1.7-.6-3.3-1.6-4.6z'
              />
            </svg>
          )}
          Regenerar
        ...
        >
          {active === 'custom' ? (
            <span
              className='size-3.5 rounded-full border-2 border-amber-900/40 border-t-transparent animate-spin'
              aria-hidden='true'
            ></span>
          ) : (
            <svg viewBox='0 0 24 24' className='size-4' aria-hidden='true'>
              <path
                fill='currentColor'
                d='M3 6a3 3 0 013-3h7l5 5v10a3 3 0 01-3 3H6a3 3 0 01-3-3V6zm12 0H6a1 1 0 00-1 1v10c0 .6.4 1 1 1h10a1 1 0 001-1V9h-2a1 1 0 01-1-1V6z'
              />
            </svg>
          )}
          Escribir la mía
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (scheduleMode) {
        ...
        const scheduledTime = selectedDateTime
          .toTimeString()
          .split(' ')[0]
          .substring(0, 5); // HH:MM
  ...
  return (
    <div className='flex flex-col gap-4 rounded-xl border border-violet-200 bg-white p-4'>
      ...
      <div className='text-xs text-gray-600'>
        {publishData
          ? 'Elige cuándo publicar el contenido'
          : 'Elige la fecha y hora para programar el post'}
        . Zona horaria: <span className='font-medium'>{tz}</span>.
      </div>

      {publishStatus === 'publishing' && (
        <div className='flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg'>
          <span
            className='size-4 rounded-full border-2 border-blue-500/60 border-t-transparent animate-spin'
            aria-hidden='true'
          ></span>
          <span className='text-sm text-blue-700'>Publicando...</span>
        </div>
      )}
