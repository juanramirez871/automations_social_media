      ...
      <div className='grow text-end space-y-3'>
        ...
        <ul className='flex flex-col justify-end text-start -space-y-px'>
          {files.map(name => (
            <li
              key={name}
              className='flex items-center gap-x-2 p-3 text-sm bg-white border border-gray-200 text-gray-600 first:rounded-t-lg first:mt-0 last:rounded-b-lg'
            >
              <div className='w-full flex justify-between truncate'>
                <span className='me-3 flex-1 w-0 truncate'>{name}</span>
                <button
                  type='button'
                  className='flex items-center gap-x-2 text-gray-500 hover:text-blue-500 focus:outline-hidden focus:text-blue-500 whitespace-nowrap cursor-pointer'
                >
                  <svg
