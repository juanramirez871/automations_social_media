...
export async function GET(request) {
  ...
  const rawScopes = process.env.YOUTUBE_OAUTH_SCOPES || "https://www.googleapis.com/auth/youtube.upload https://www.googleapis.com/auth/youtube.readonly";
  const accessType = process.env.GOOGLE_OAUTH_ACCESS_TYPE || "offline";
  const prompt = process.env.GOOGLE_OAUTH_PROMPT || "consent";

  if (!clientId || !redirectUri) {
    return NextResponse.json({ error: "Missing GOOGLE_CLIENT_ID or GOOGLE_OAUTH_REDIRECT_URI" }, { status: 500 });
  }
