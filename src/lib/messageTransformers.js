...
export function transformMessagesFromDB(rawMessages) {
  ...
  const transformedMessages = rawMessages.map((r) => {
    const rType = r.type;
    ...
    if (rType === "widget-instagram-credentials" || rType === "widget-instagram-auth") {
    ...
    if (rType === "widget-youtube-connected") {
      ...
      return { id: r.id, role: "assistant", type: "widget-youtube-connected", channelId, channelTitle, grantedScopes };
    }
    ...
    if (rType === "widget-tiktok-connected") {
      ...
      return { id: r.id, role: "assistant", type: "widget-tiktok-connected", displayName, username, avatarUrl };
    }
    ...
    if (rType === "widget-await-media") {
      ...
      return { id: r.id, role: "assistant", type: "widget-await-media", meta: { targets } };
    }
    
    if (rType === "widget-caption-suggest") {
      const caption = r?.meta?.caption || "";
      return { id: r.id, role: "assistant", type: "widget-caption-suggest", meta: { caption } };
    }
